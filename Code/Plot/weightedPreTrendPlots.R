## parallel trends visualisation, weighted
library(tidyverse)
library(patchwork)
...
subBirth <- df %>%
  select(YEAR, WTFINL, STATECENSUS, YEARCAPPED, YEARREPEALED, BIRTHi) %>%
...
cohorts <- c(1992, 1994, 1995, 1996, 1997, 1998, 2003)

df1992 <- subBirth %>%
  filter(YEARCAPPED == 0 | YEARCAPPED == 1992) %>%
  filter(YEAR <= 1997 & YEAR >= 1987) %>%
  group_by(YEAR, STATECENSUS) %>%
  summarise(pBirth = weighted.mean(BIRTHi, WTFINL),
...
p1992 <- ggplot(df1992, aes(x = YEAR, y = means, group = as.factor(Group), colour = as.factor(Group))) +
  ...
  labs(y = "Percent of sample that gave birth", x = "Year") +
  theme_minimal()+
